...
import { ShieldAlert, Key, User, ArrowRight, Smartphone, RefreshCw, CheckCircle, Eye, EyeOff } from 'lucide-react';
...
export default function AdminLogin2FA({ onLoginSuccess, onBackToStore }: AdminLogin2FAProps) {
  ...
  // Handle first stage: Login Credentials
  const handleCredentialsSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim() === 'admin' && password === 'elizabethgallery123') {
      ...
      setError('Kredensial salah! Gunakan username: admin dan password: elizabethgallery123');
    }
  };
  ...
  const handle2FASubmit = (e: React.FormEvent) => {
    ...
    if (totpCode.replace(/\s+/g, '') === current2FACode) {
      ...
      setError('Kode 2FA salah atau telah kedaluwarsa! Silakan periksa Authenticator Simulator di samping.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden" id="admin-login-screen">
      ...
      <div className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row min-h-[500px]">
        {/* Left Side: Brand presentation */}
        <div className="md:w-1/2 bg-[#0B132B] p-8 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-800 text-center space-y-4">
          <Logo size="lg" showDetails={false} />
          <div className="space-y-1.5">
            <h3 className="text-amber-400 font-serif text-xl italic font-bold">Portal Administrasi Toko</h3>
            <p className="text-xs text-slate-400 px-6 max-w-xs mx-auto leading-relaxed">
              Selamat datang di sistem manajemen penjualan Elizabeth Signature Gallery. Lindungi akses dengan kredensial & otentikasi lapis ganda.
            </p>
          </div>
          <button 
            onClick={onBackToStore}
            className="text-xs text-slate-400 hover:text-white hover:bg-slate-800 border border-slate-800 px-4 py-2 rounded-xl transition duration-200 mt-4 cursor-pointer"
          >
        ...
        <div className="md:w-1/2 p-8 flex flex-col justify-center bg-slate-900 text-white">
          {step === 'CREDENTIALS' ? (
            <div className="space-y-6">
              <div className="space-y-1">
                <div className="flex items-center gap-2 text-amber-500">
                  <ShieldAlert size={18} />
                  <span className="text-xs font-bold uppercase tracking-wider">Akses Terproteksi</span>
                </div>
                <h2 className="text-2xl font-bold font-serif tracking-tight text-white">Login Administrator</h2>
                <p className="text-xs text-slate-400">Masukkan akun admin Elizabeth Signature Anda.</p>
              </div>
              ...
              <form onSubmit={handleCredentialsSubmit} className="space-y-4">
                ...
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800/80 text-[11px] text-slate-500 space-y-1 leading-normal">
                  <p className="font-bold text-slate-400 uppercase tracking-widest text-[10px]">💡 Akun Demo default:</p>
                  <p>Username: <strong className="text-slate-300">admin</strong></p>
                  <p>Password: <strong className="text-slate-300">elizabethgallery123</strong></p>
                </div>
            ...
            <div className="space-y-6">
              <div className="space-y-1">
                <div className="flex items-center gap-2 text-teal-400">
                  <Smartphone size={18} />
                  <span className="text-xs font-bold uppercase tracking-wider">Lapis Keamanan Kedua</span>
                </div>
                <h2 className="text-2xl font-bold font-serif tracking-tight text-white">Verifikasi Otentikasi 2FA</h2>
                <p className="text-xs text-slate-400">Masukkan 6 digit kode keamanan dari aplikasi authenticator Anda.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-5 items-start">
                ...
                {/* Form input - 7 cols */}
                <form onSubmit={handle2FASubmit} className="sm:col-span-7 space-y-4">
                  <div className="space-y-2">
                    <label className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Kode Keamanan 6-Digit</label>
                    <input
                      type="text"
                      required
                      maxLength={6}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold py-2.5 px-3 rounded-lg transition"
                    >
                      Kembali
                    </button>
                    <button
                      type="submit"
                      disabled={success}
                      className="flex-1 bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 text-xs font-extrabold py-2.5 px-3 rounded-lg transition cursor-pointer"
                    >
                      Verifikasi & Masuk
                    </button>
                  </div>
                ...
                <div className="sm:col-span-5 bg-slate-950 rounded-2xl border border-slate-800 p-4 space-y-3 shadow-inner">
                  ...
                  <div className="text-center space-y-1.5">
                    <p className="text-[10px] text-slate-500">Elizabeth Admin Key</p>
                    <div 
                      onClick={autofill2FACode}
                      className="bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-lg py-2 cursor-pointer transition active:scale-95"
                    >
                      <span className="text-xl font-mono font-bold text-teal-400 tracking-wider">
                    ...
                    <div className="space-y-1 text-right">
                      ...
                      <span className="text-[9px] text-slate-500 font-mono">Kode berputar dalam {secondsLeft}s</span>
                    </div>
                  </div>

                  <p className="text-[9px] text-slate-500 leading-normal">
                    *Simulator ini merepresentasikan integrasi Google/Microsoft Authenticator MFA untuk melindungi halaman admin Anda secara maksimal.
                  </p>
                </div>
