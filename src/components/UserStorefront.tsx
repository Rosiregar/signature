...
import { 
  ShoppingBag, Search, Filter, ShoppingCart, X, Plus, Minus, 
  Trash2, CreditCard, ChevronRight, CheckCircle2, MapPin, 
  Phone, Instagram, Clock, Gift, Heart, HelpCircle, ArrowRight
} from 'lucide-react';
...
interface UserStorefrontProps {
  ...
  onGoToAdmin: () => void;
  onOpenEmails: () => void;
  onSendCustomRequest: (customerName: string, customerEmail: string, details: string) => void;
}
...
export default function UserStorefront({
  ...
  const [searchedTrackOrder, setSearchedTrackOrder] = useState<Order | null>(null);
  ...
  const handleTrackSearch = (e: React.FormEvent) => {
    ...
    if (found) {
      ...
      setTrackError('ID Pesanan tidak ditemukan! Pastikan Anda memasukkan kode ID yang benar (contoh: ESG-1001).');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-stone-800 font-sans" id="user-storefront-layout">
      ...
      {activeOrder && (
        <div className="bg-[#F8F5F2] border-y border-stone-200 px-6 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-stone-900">
          <p className="font-semibold flex items-center gap-1.5">
            ...
            <span>Pesanan Anda saat ini sedang dalam proses pembayaran: <strong>#{activeOrder.id}</strong> ({formatIDR(activeOrder.total)})</span>
          </p>
      ...
      <main className="min-h-[500px]">
        {activeTab === 'SHOP' ? (
          <div className="animate-fade-in space-y-12">
            ...
            <section className="relative bg-[#5B745C] text-white py-20 px-8 text-center overflow-hidden border-b-8 border-[#D4A373]" id="storefront-hero">
              {/* Blur Backdrops */}
              ...
              <div className="max-w-3xl mx-auto space-y-6 relative z-10">
                ...
                <p className="text-xs md:text-sm text-stone-100 max-w-xl mx-auto leading-relaxed">
                  Elizabeth Signature Gallery merangkai mawar segar pilihan, lily, dan tulip import dengan sentuhan seni tingkat tinggi. Sempurna untuk romansa, hari wisuda, pernikahan, maupun kado ulang tahun premium.
                </p>
                <div className="pt-2">
                  <a 
            ...
            {/* Catalog Container */}
            <section className="max-w-7xl mx-auto px-6 py-8 space-y-8" id="catalog-container">
              ...
              {processedProducts.length === 0 ? (
                <div className="text-center py-20 bg-[#F8F5F2] border border-stone-200 rounded-3xl space-y-3">
                  <ShoppingBag size={48} className="mx-auto text-stone-300 stroke-1 animate-bounce" />
                  <p className="text-stone-800 font-bold">Produk Tidak Ditemukan</p>
                  <p className="text-xs text-stone-500">Coba kata kunci pencarian yang lain atau ganti kategori.</p>
                </div>
              ) : (
            ...
            <section className="max-w-7xl mx-auto px-6 py-6" id="custom-design-section">
              <div className="bg-gradient-to-br from-[#5B745C] to-[#4d634e] text-white rounded-3xl p-8 md:p-12 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 border-b-8 border-[#D4A373]">
                ...
                <div className="space-y-4 max-w-2xl relative z-10 text-left">
                  ...
                  <p className="text-xs md:text-sm text-stone-100 leading-relaxed">
                    Setiap momen wisuda, ulang tahun, lamaran, atau pernikahan adalah cerita unik. Elizabeth Signature menyediakan <strong>Buket Bunga Artificial / Imitasi Premium</strong> (satin, felt, soap, dried, latex) yang tetap cantik selamanya tanpa layu. Anda bebas memilih kombinasi warna mawar, bahan kertas pembungkus (wrapping), hingga menyesuaikan budget impian Anda!
                  </p>
                  <div className="flex flex-wrap gap-4 text-xs font-semibold text-stone-100/90 pt-1 text-left">
                    <span className="flex items-center gap-1.5 bg-stone-900/10 py-1 px-2.5 rounded-lg border border-white/10">✓ Kelopak Sutra & Latex Real-Touch</span>
                    <span className="flex items-center gap-1.5 bg-stone-900/10 py-1 px-2.5 rounded-lg border border-white/10">✓ Custom Warna & Wrap</span>
                    <span className="flex items-center gap-1.5 bg-stone-900/10 py-1 px-2.5 rounded-lg border border-white/10">✓ Tahan Selamanya</span>
                  </div>
                </div>
          ...
          <div className="max-w-3xl mx-auto px-6 py-12 animate-fade-in space-y-8" id="track-order-tab">
            <div className="text-center space-y-2">
              <h2 className="text-3xl font-serif italic font-bold text-stone-900">Lacak Status Pesanan Anda</h2>
              <p className="text-xs text-stone-500">Ketikkan nomor ID Pesanan unik Anda untuk memantau status secara live.</p>
            </div>

            {/* Tracking Search Input */}
            <form onSubmit={handleTrackSearch} className="bg-[#F8F5F2] p-6 rounded-3xl border border-stone-200/80 flex items-center gap-3">
              <input
                type="text"
                required
                value={trackOrderId}
            ...
            {searchedTrackOrder ? (
              <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-md animate-zoom-in space-y-6 p-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-stone-100 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[10px] text-stone-400 font-semibold uppercase font-mono">Kode Pesanan</span>
                      <h3 className="text-lg font-mono font-extrabold text-[#D4A373]">#{searchedTrackOrder.id}</h3>
                    </div>
                  </div>

                  <div className="text-right">
                    <span className="text-[10px] text-stone-400 font-semibold uppercase block">Waktu Dibuat</span>
                    <span className="text-xs text-stone-700 font-bold">
                      {new Date(searchedTrackOrder.createdAt).toLocaleString('id-ID')}
                    </span>
                  </div>
                </div>
                ...
                <div className="py-4 space-y-6">
                  <h4 className="text-xs font-bold text-[#5B745C] uppercase tracking-wider">Status Pengantaran Live:</h4>
                  ...
                  <div className="relative pl-8 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-1 before:bg-stone-250">
                    ...
                    <div className="relative">
                      <div className={`absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center border-2 text-[10px] font-bold ${
                      ...
                      <div>
                        <p className="text-xs font-bold text-stone-900">Pesanan Masuk</p>
                        <p className="text-[11px] text-stone-500">
                          {searchedTrackOrder.status === 'CANCELLED' ? 'Pesanan dibatalkan' : 'Menunggu pemrosesan pembayaran otomatis.'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      <div className={`absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center border-2 text-[10px] font-bold ${
                        ['PAID', 'PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                          ? 'bg-emerald-500 border-emerald-500 text-white'
                          : 'bg-white border-stone-300 text-stone-400'
                      }`}>
                        {['PAID', 'PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status) ? '✓' : '2'}
                      </div>
                      <div>
                        <p className="text-xs font-bold text-stone-900">Pembayaran Terverifikasi (Lunas)</p>
                        <p className="text-[11px] text-stone-500">
                          {['PAID', 'PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                    ...
                    <div className="relative">
                      <div className={`absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center border-2 text-[10px] font-bold ${
                        ['PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                          ? 'bg-emerald-500 border-emerald-500 text-white'
                          : 'bg-white border-stone-300 text-stone-400'
                      }`}>
                        {['PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status) ? '✓' : '3'}
                      </div>
                      <div>
                        <p className="text-xs font-bold text-stone-900">Sedang Dirangkai oleh Florist</p>
                        <p className="text-[11px] text-stone-500">
                          {['PROCESSED', 'SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                    ...
                    <div className="relative">
                      <div className={`absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center border-2 text-[10px] font-bold ${
                        ['SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                          ? 'bg-emerald-500 border-emerald-500 text-white'
                          : 'bg-white border-stone-300 text-stone-400'
                      }`}>
                        {['SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status) ? '✓' : '4'}
                      </div>
                      <div>
                        <p className="text-xs font-bold text-stone-900">Sedang Dikirim oleh Kurir</p>
                        <p className="text-[11px] text-stone-500">
                          {['SHIPPED', 'COMPLETED'].includes(searchedTrackOrder.status)
                    ...
                    <div className="relative">
                      ...
                      <div>
                        <p className="text-xs font-bold text-stone-900">Pesanan Selesai (Diterima)</p>
                        <p className="text-[11px] text-stone-500">
                          {searchedTrackOrder.status === 'COMPLETED'
                ...
                <div className="border-t border-stone-100 pt-5 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-stone-600">
                    <div className="bg-[#F8F5F2] p-4 rounded-2xl border border-stone-200">
                      <p className="font-bold text-stone-800 uppercase text-[10px] mb-2 tracking-wider">Item Buket:</p>
                      {searchedTrackOrder.items.map((it, idx) => (
                        <div key={idx} className="flex justify-between items-center py-1 border-b border-stone-200/50">
                          <span>{it.quantity}x {it.product.name}</span>
                          <span className="font-bold text-stone-900 font-mono">{formatIDR(it.product.price * it.quantity)}</span>
                        </div>
                      ))}
                      <div className="flex justify-between items-center font-bold text-[#5B745C] pt-3 text-sm">
                        <span>Total Terbayar:</span>
                        <span className="font-extrabold font-mono text-stone-950">{formatIDR(searchedTrackOrder.total)}</span>
                      </div>
                    </div>

                    <div className="bg-[#F8F5F2] p-4 rounded-2xl border border-stone-200 space-y-2">
                      <p className="font-bold text-stone-800 uppercase text-[10px] tracking-wider">Lokasi Pengantaran:</p>
                      <p><strong>Nama:</strong> {searchedTrackOrder.customerName}</p>
                      <p><strong>No. HP:</strong> {searchedTrackOrder.customerPhone}</p>
                      <p><strong>Alamat:</strong> {searchedTrackOrder.customerAddress}</p>
                      <p><strong>Pesan Kartu:</strong> <span className="italic">"{searchedTrackOrder.notes || 'Tidak ada pesan'}"</span></p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-[#F8F5F2] p-6 rounded-3xl text-center space-y-4 border border-stone-200">
                  <p className="text-xs text-stone-500 font-bold uppercase tracking-widest">💡 Demo Shortcut - Lacak Pesanan Pengujian:</p>
                  {orders.length === 0 ? (
                    <p className="text-xs text-stone-400">Belum ada pesanan terdaftar di sistem. Silakan berbelanja di katalog dulu!</p>
                  ) : (
                    <div className="flex flex-wrap gap-2.5 justify-center">
                      {orders.slice(0, 4).map((o) => (
                        ...
                        >
                          ...
                          <span className="text-[10px] bg-stone-100 px-1.5 py-0.5 rounded text-stone-600 uppercase">{o.status}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-xs space-y-4">
                  <div className="flex items-center gap-2 text-[#5B745C]">
                    <HelpCircle size={18} />
                    <h3 className="font-bold text-sm text-stone-900">Lupa Kode Pesanan (ID Pesanan) Anda?</h3>
                  </div>
                  <p className="text-xs text-stone-500 leading-relaxed">
                    Jangan khawatir! Masukkan alamat <strong>Email</strong> atau <strong>Nomor WhatsApp / HP</strong> yang Anda gunakan saat memesan untuk menemukan kode pesanan Anda secara otomatis:
                  </p>
                  ...
                  {forgotContactQuery.trim() !== '' && (
                    <div className="space-y-3 pt-2">
                      <p className="text-[11px] font-bold text-stone-600 uppercase tracking-wider">Hasil Pencarian Pesanan:</p>
                      {(() => {
                        ...
                        if (matches.length === 0) {
                          return (
                            <p className="text-xs text-stone-400 italic bg-[#F8F5F2] p-4 rounded-2xl text-center">
                              Tidak ditemukan pesanan yang cocok dengan kontak "{forgotContactQuery}". Silakan periksa kembali email atau nomor HP Anda.
                            </p>
                          );
                        }
                            ...
                            {matches.map((o) => (
                              ...
                              >
                                <div className="space-y-0.5">
                                  <div className="flex items-center gap-2">
                                    <span className="font-mono font-bold text-[#D4A373]">#{o.id}</span>
                                    <span className="text-[10px] bg-stone-200 px-1.5 py-0.5 rounded text-stone-700 font-bold uppercase">
                                      {o.status}
                                    </span>
                                  </div>
                                  <p className="text-stone-700 font-medium">An. {o.customerName} ({o.customerPhone})</p>
                                  <p className="text-[10px] text-stone-400">{new Date(o.createdAt).toLocaleString('id-ID')} • Total {formatIDR(o.total)}</p>
                                </div>
      ...
      {/* Elegant Footer */}
      <footer className="bg-[#5B745C] text-stone-100 border-t-4 border-[#D4A373] py-12 px-6 mt-16 text-xs" id="storefront-footer">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <Logo size="sm" showDetails={false} />
              <span className="font-serif italic font-extrabold text-[#D4A373] text-lg">Elizabeth Signature</span>
            </div>
            <p className="text-stone-100/90 leading-relaxed text-[11px]">
              Toko e-commerce buket bunga terkemuka di Indonesia yang memadukan keindahan premium, bunga segar pilihan, dan kemudahan pembayaran otomatis.
            </p>
          </div>

          <div className="space-y-3">
            <h4 className="font-bold text-[#D4A373] uppercase tracking-widest text-[10px]">Layanan & Koleksi</h4>
          ...
          <div className="space-y-3">
            <h4 className="font-bold text-[#D4A373] uppercase tracking-widest text-[10px]">Kontak Toko</h4>
            <ul className="space-y-2 text-stone-200 font-mono">
              <li>WhatsApp: 081344780652</li>
              <li>Instagram: @zelida00</li>
              <li>Alamat: Ruko Golden Boulevard, Blok C-12, Tangerang Selatan</li>
            </ul>
          </div>

          <div className="space-y-3">
            <h4 className="font-bold text-[#D4A373] uppercase tracking-widest text-[10px]">Otomatisasi WooCommerce</h4>
            <p className="text-stone-200 text-[11px] leading-relaxed">
              Sistem ini mendemonstrasikan integrasi payment gateway lokal (QRIS, VA Transfer), auto-deduction stock, sinkronisasi dasbor analitik admin, dan trigger SMTP email otomatis.
            </p>
          </div>
        ...
        <div className="max-w-7xl mx-auto border-t border-[#D4A373]/30 mt-10 pt-6 text-center text-stone-200/75 flex justify-center items-center">
          <p>© 2026 Elizabeth Signature Gallery. Crafted elegantly with deep local Indonesian integration.</p>
        </div>
      </footer>

      {/* CART DRAWER BACKDROP AND ELEMENT */}
      {isCartOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex justify-end" id="cart-drawer-overlay">
          <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in">
            ...
            <div className="bg-[#0B132B] text-white px-6 py-5 flex items-center justify-between border-b border-[#C5A059]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold font-serif italic text-md text-amber-100">Keranjang Belanja Anda</h3>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 min-h-0">
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-400">
                  <ShoppingCart size={48} className="stroke-1 mb-2 text-slate-300" />
                  <p className="text-sm font-bold">Keranjang Anda Masih Kosong</p>
                  <p className="text-xs px-6 mt-1">Tambahkan buket bunga cantik dari katalog toko untuk melanjutkan pemesanan.</p>
                </div>
              ) : (
                  ...
                  {cart.map((item) => (
                    <div key={item.product.id} className="py-4 flex gap-3 first:pt-0">
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        <p className="text-[10px] text-slate-400 uppercase font-semibold">{item.product.category}</p>
                        <p className="text-xs font-extrabold text-slate-950 font-mono">{formatIDR(item.product.price)}</p>
            ...
            {cart.length > 0 && (
              <div className="bg-slate-50 p-6 border-t border-slate-100 space-y-4">
                <div className="flex justify-between items-center text-slate-800">
                  <span className="text-xs font-bold uppercase text-slate-500">Subtotal Belanja:</span>
                  <span className="text-xl font-extrabold font-mono text-slate-950">{formatIDR(cartSubtotal)}</span>
                </div>
                <div className="text-[10px] text-slate-400 leading-normal bg-white p-2.5 rounded-lg border border-slate-200/60">
                  🎁 Flat Rate Shipping: Kami menyediakan layanan pengantaran GRATIS (Gratis Ongkir) ke seluruh wilayah Kota Tangerang & Jakarta Selatan.
                </div>
                <button
      ...
      {isCheckoutOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4" id="checkout-modal-form">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-zoom-in border border-slate-100">
            <div className="bg-[#0B132B] p-5 text-white flex items-center justify-between border-b border-[#C5A059]">
              <h3 className="font-bold text-md font-serif italic text-amber-100">Form Pemesanan & Alamat Pengiriman</h3>
              <button 
            ...
            <form onSubmit={handlePlaceOrder} className="p-6 space-y-4 max-h-[500px] overflow-y-auto text-xs text-slate-700">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 font-bold uppercase">Nama Pemesan / Pembeli</label>
                  <input
                    type="text"
                    required
                    value={custName}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 font-bold uppercase">Email Aktif (Notifikasi Otomatis)</label>
                  <input
                    type="email"
                    required
                    value={custEmail}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 font-bold uppercase">No. WhatsApp Aktif</label>
                  <input
                    type="text"
                    required
                    value={custPhone}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 font-bold uppercase">Metode Pembayaran Gateway</label>
                  <select
                    value={payMethod}
                  ...
                  >
                    <option value="QRIS">QRIS Otomatis (Dana, OVO, GoPay, LinkAja)</option>
                    <option value="VA_BCA">Virtual Account BCA (Verifikasi Otomatis)</option>
                    <option value="VA_MANDIRI">Virtual Account Mandiri (Verifikasi Otomatis)</option>
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-400 font-bold uppercase">Alamat Lengkap Pengiriman Buket</label>
                <textarea
                  rows={2}
                  required
                  value={custAddress}
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-400 font-bold uppercase">Pesan Khusus di Kartu Ucapan (Gift Card Message)</label>
                <textarea
                  rows={2}
                  value={cardMessage}
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-2">
                <p className="font-bold text-slate-800 uppercase text-[9px] tracking-wider">Ringkasan Tagihan:</p>
                <div className="flex justify-between items-center text-[11px]">
                  <span className="text-slate-500">Total Harga Bunga:</span>
                  <span className="font-bold text-slate-900 font-mono">{formatIDR(cartSubtotal)}</span>
                </div>
                <div className="flex justify-between items-center text-[11px]">
                ...
                <div className="flex justify-between items-center text-xs font-bold text-slate-900 border-t border-slate-200 pt-2">
                  ...
                  <span className="font-extrabold font-mono text-slate-950">{formatIDR(cartSubtotal)}</span>
                </div>
              </div>
      ...
      {isCustomRequestOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-fade-in" id="custom-request-modal">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-zoom-in border border-stone-200">
            <div className="bg-[#5B745C] p-5 text-white flex items-center justify-between border-b-4 border-[#D4A373]">
              <div>
                ...
                <p className="text-[10px] text-stone-200 uppercase tracking-widest mt-0.5">Bunga Artificial & Material Premium</p>
              </div>
              <button 
            ...
            <form onSubmit={handleCustomRequestSubmit} className="p-6 space-y-4 max-h-[480px] overflow-y-auto text-xs text-stone-700 text-left">
              <div className="bg-[#F8F5F2] p-4 rounded-2xl border border-stone-200 text-[11px] leading-relaxed text-stone-600 mb-2">
                <strong>💡 Catatan Kustomisasi:</strong> Anda bebas meminta desain model apa saja, perpaduan bunga artificial (satin/latex/silk/dried), warna kertas pembungkus, aksen pita, hingga penulisan kartu ucapan khusus sesuai selera.
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] text-stone-400 font-bold uppercase">Nama Anda</label>
                  <input
                    type="text"
                    required
                    value={customName}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-stone-400 font-bold uppercase">Email Aktif</label>
                  <input
                    type="email"
                    required
                    value={customEmail}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] text-stone-400 font-bold uppercase">No. WhatsApp Aktif</label>
                  <input
                    type="text"
                    required
                    value={customPhone}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-stone-400 font-bold uppercase">Acara / Momen Spesial</label>
                  <select
                  ...
                  >
                    ...
                    <option value="Ulang Tahun / Birthday">Ulang Tahun / Birthday</option>
                    <option value="Lamaran / Proposal">Lamaran / Proposal</option>
                    <option value="Hari Ibu / Mother's Day">Hari Ibu / Mother's Day</option>
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-stone-400 font-bold uppercase">Anggaran / Target Budget (IDR)</label>
                <select
                  value={customBudget}
                ...
                >
                  ...
                  <option value="800000">Rp 800.000 (Royal Lux Exclusive)</option>
                  <option value="1500000">Rp 1.500.000+ (Grand Masterpiece)</option>
                </select>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] text-stone-400 font-bold uppercase">Deskripsi Ide Desain Pilihan Anda</label>
                <textarea
                  rows={4}
                  required
                  value={customDetails}
